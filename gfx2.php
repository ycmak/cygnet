...
$paksux = 0;
	function walk($goal,$elapsed,$effect,$maxtime){
		global $link,$links,$link_info,$_GET,$nodes,$max,$donemonth,$exofacs,$paksux,$n2m;
		if(isset($nodes[$goal])) $nodes[$goal] += ($effect/5); // linear scale, additive effect
		else $nodes[$goal] = $effect;
		if(abs($nodes[$goal])>$max) $max = abs($nodes[$goal]);
		# CHECK GLOBAL CALENDAR

		if(isset($donemonth[$elapsed])&&!$donemonth[$elapsed]){
			$donemonth[$elapsed] = true;
			//$paksux = count($exofacs[2]);
			for($k=0;$k<count($exofacs[$elapsed]);$k++){

				//$paksux = $links[$exofacs[$elapsed][$k]['node_id']][$j]['to'];
				for($j=0;$j<count($links[$exofacs[$elapsed][$k]['node_id']]);$j++){
					//$paksux = '('.$links[$exofacs[$elapsed][$k]['node_id']][$j]['to'].','.$elapsed.','.$exofacs[$elapsed][$k]['mag'].','.$maxtime.')';
					$nodes[$exofacs[$elapsed][$k]['node_id']] += ($n2m[$exofacs[$elapsed][$k]['node_id']]/5); // ***asd***
					walk($links[$exofacs[$elapsed][$k]['node_id']][$j]['to'],$elapsed,($link_info[$links[$exofacs[$elapsed][$k]['node_id']][$j]['id']]['magnitude']/5*$exofacs[$elapsed][$k]['mag']),$maxtime);
					//$paksux = $links[$exofacs[$elapsed][$k]['node_id']][$j]['to'];
				}
			}
		}
		...
		*/
		...
		for($x=1;$x<=$_GET['z'];$x++){
			...
			for($t=0;$t<count($links[$_GET['nodec'.$x]]);$t++){
				walk($links[$_GET['nodec'.$x]][$t]['to'],0,($link_info[$links[$_GET['nodec'.$x]][$t]['id']]['magnitude']/5*$_GET['mag'.$x]),$_GET['maxtime']);
			}
		} // repeat this loop
